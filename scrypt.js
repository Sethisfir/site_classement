...
function tri (){										//etablissement fonction tri//

	var array = liste.concat(liste2);                   //fusion des deux listes//
	var tab = new Array;								//creation Nouveau tableau//
	var indice;											//variable indice de reinitialisation//
	var box;											//box mémoire//
	while (array.length != 0){							//tant que la taille du tableau n'est pas égale a 0//
		min=array[0][2];								//établissement de valeur min à array[0][2]//

	for(i=0;i<array.length;i++){						//boucle//
		if (array[i][2]<=min){							//si array[i][2] plus petit que min//
			min = array[i][2];							//donner valeur array[i][2] à min//
			box = array[i];								//enregistrer array[i] en memoire//
			indice = i;									//reinitialiser i//
		}
	}
	tab.push(box);										//push memoire dans le nouveau tableau//
	array.splice(indice, 1);							//retirer valeur enregistrer du tableau d'origine//
	}
	document.getElementById("p").innerHTML = tab;		//afficher nouveau tableau//
}

function triOrdre (){

	var array = liste.concat(liste2);
	var tab = new Array;
	var indice;
	var box;
	...
	document.getElementById("p").innerHTML = tab;
}
